...
function Build-Item([string]$ItemId) {
    ...
    return [ordered]@{
        ...
        eissn = if ($fields.eISSN) { $fields.eISSN } elseif ($fields.eissn) { $fields.eissn } else { "" }
        isbn = if ($fields.ISBN) { $fields.ISBN } elseif ($fields.isbn) { $fields.isbn } else { "" }
        udc = if ($fields.UDC) { $fields.UDC } elseif ($fields.udc) { $fields.udc } else { "" }
        lccn = if ($fields.callNumber) { $fields.callNumber } else { "" }
        grnti = if ($fields.GRNTI) { $fields.GRNTI } elseif ($fields.grnti) { $fields.grnti } else { "" }
        edn = if ($fields.EDN) { $fields.EDN } elseif ($fields.edn) { $fields.edn } else { "" }
        pmid = if ($fields.PMID) { $fields.PMID } elseif ($fields.pmid) { $fields.pmid } else { "" }
        pmcid = if ($fields.PMCID) { $fields.PMCID } elseif ($fields.pmcid) { $fields.pmcid } else { "" }
        arxiv = if ($fields.arXiv) { $fields.arXiv } elseif ($fields.arxiv) { $fields.arxiv } else { "" }
        keywords = if ($fields.tags) { $fields.tags } elseif ($fields.keywords) { $fields.keywords } else { "" }
        access_date = if ($fields.accessDate) { $fields.accessDate } else { "" }
        edition = if ($fields.edition) { $fields.edition } else { "" }
        series = if ($fields.series) { $fields.series } else { "" }
        series_number = if ($fields.seriesNumber) { $fields.seriesNumber } else { "" }
        library_catalog = if ($fields.libraryCatalog) { $fields.libraryCatalog } else { "" }
        rights = if ($fields.rights) { $fields.rights } else { "" }
